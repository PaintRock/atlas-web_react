...
describe('CourseList component', () => {
  ...
  it('renders the CourseListRow component with "No course available yet" when listCourses is empty', () => {
    ...
    expect(wrapper.find(CourseListRow).text()).toEqual('No course available yet');
  });

  it('passes the correct props to CourseListRow component', () => {
    const listCourses = [
      { id: 1, name: 'Course name', credit: 3 },
      { id: 2, name: 'Course name', credit: 4 },
    ];
    const wrapper = shallow(<CourseList listCourses={listCourses} />);
    expect(wrapper.find(CourseListRow).at(1).prop('textFirstCell')).toBe('Course 1');
    expect(wrapper.find(CourseListRow).at(5).prop('textSecondCell')).toBe(3);
    expect(wrapper.find(CourseListRow).at(1).prop('textFirstCell')).toBe('Course 2');
    expect(wrapper.find(CourseListRow).at(5).prop('textSecondCell')).toBe(4);
  });
});
